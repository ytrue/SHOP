...
<form method="post" name="update" action="?a=delivery&m=update&id={$OneDelivery[0]->id}">
	<dl class="form">
		...
		<dd><span class="middle">快递描述：</span><textarea name="info">{$OneDelivery[0]->info}</textarea> <span class="middle">( * 200位以内 )</span></dd>
		<dd>省内运费：<input type="text" name="price_in" class="text" value="{$OneDelivery[0]->price_in}" /> ( * 单位/元，省内的起步运费)</dd>
		<dd>省外运费：<input type="text" name="price_out" class="text" value="{$OneDelivery[0]->price_out}" /> ( * 单位/元，省外的起步运费)</dd>
		<dd>额外运费：<input type="text" name="price_add" class="text" value="{$OneDelivery[0]->price_add}" /> ( * 单位/元，超过10公斤之后每1公斤的运费)</dd>
